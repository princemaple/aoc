...
# def part1(color, seen, lookup):
#     print(lookup[color])
#     for c in lookup[color]:
#         if c in seen:
#             continue
#         seen.add(c)
#         part1(c, seen, lookup)
...
total = 0
def part2(color, lookup, factor):
    global total
    if not lookup[color]:
        return

    for count, color in lookup[color]:
        total += count * factor
        part2(color, lookup, count * factor)
...
def main():
    ...
    pat_top = re.compile(r'^((?:\w+ )+)bags contain (.+)$')
    pat_sub = re.compile(r'(\d+) ((?:\w+ )+)bags?|no other bags')

    # in_to_out = defaultdict(set)
    out_to_in = defaultdict(list)
    # trace = set()

    for x in data:
        source, sinks = re.match(pat_top, x).groups()
        source = source.strip()
        sinks = sinks.strip('.').split(', ')
        sinks = [re.match(pat_sub, sink).groups() for sink in sinks]

        for count, color in sinks:
            if color is None:
                continue
            out_to_in[source].append((int(count), color.strip()))

    # part1('shiny gold', trace, in_to_out)
    # print(len(trace))

    part2('shiny gold', out_to_in, 1)
    print(total)
