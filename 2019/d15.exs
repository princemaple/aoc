...
defmodule IntCode do
  ...
  defp exec([3 | _], _m, _i, nil, _base) do
    raise "wrong"
  end
  ...
  defp exec([4, mode | _], m, i, _input, base) do
...
defmodule Solver do
  def build_map({program, index, base}, {x, y}, map) do
    1..4
    |> Enum.reduce(map, fn input, map ->
      {x, y} =
        case input do
          1 -> {x, y - 1}
          2 -> {x, y + 1}
      ...
      if map[{x, y}] do
        ...
        {output, {program, index, base}} = IntCode.run(program, index, input, base)
  ...
  def walk(_map, _step, [], [], done) do
    done
  end

  def walk(map, step, [], backlog, done) do
    walk(map, step + 1, backlog, [], done)
  end

  def walk(map, step, [{x, y} | todo], backlog, done) do
    to_backlog =
      [{x - 1, y}, {x + 1, y}, {x, y - 1}, {x, y + 1}]
      |> Enum.filter(& !(Map.get(done, &1)))
      |> Enum.filter(& Map.get(map, &1) != 0)

    walk(map, step, todo, to_backlog ++ backlog, Map.put(done, {x, y}, step))
  end
end
...
program = t |> Parser.parse_program
map = Solver.build_map({program, 0, 0}, {0, 0}, %{})
...
{coords, _} = Enum.find(map, fn {_coords, type} -> type == 2 end)
IO.inspect Solver.walk(map, 0, [{0, 0}], [], %{})[coords], label: "part1"
IO.inspect elem(Enum.max_by(Solver.walk(map, 0, [coords], [], %{}), fn {_coords, step} -> step end), 1), label: "part2"
