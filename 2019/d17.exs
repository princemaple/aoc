...
defmodule IntCode do
  ...
  defp exec([3 | _], _m, _i, [], _base) do
    raise "wrong"
  end

  defp exec([3, mode | _], m, i, [input | rest], base) do
    w = deref(mode, m, i + 1, base, :pointer)
    run(Map.put(m, w, input), i + 2, rest, base)
  end

  defp exec([4, mode | _], m, i, input, base) do
    output = deref(mode, m, i + 1, base)
    {output, {m, i + 2, input, base}}
  end
...
defmodule Solver do
  def build_map(t) do
    program = Parser.parse_program(t)
    map = read_map({program, 0, 0}, [], [])
    IO.puts map

    map
    |> String.trim
    |> String.split("\n", trim: true)
    |> Enum.map(&(&1 |> String.split("", trim: true) |> Enum.with_index))
    |> Enum.with_index
    |> Enum.reduce(%{}, fn {list, y}, acc ->
      Enum.reduce(list, acc, fn {cell, x}, acc ->
        Map.put(acc, {x, y}, cell)
      end)
    end)
  end

  def read_map({program, pointer, base}, input, outputs) do
    case IntCode.run(program, pointer, input, base) do
      {output, {program, pointer, input, base}} ->
        read_map({program, pointer, base}, input, [output | outputs])
      {_program, _pointer, _base} ->
        outputs
        |> Enum.reverse
        |> to_string
    end
  ...
  def calc1(map) do
    ...
    |> Enum.filter(fn {x, y} ->
      if map[{x, y}] == "#" do
        Enum.all?([{x - 1, y}, {x + 1, y}, {x, y - 1}, {x, y + 1}], fn coord ->
          map[coord] == "#"
        end)
      end
    end)
  ...
  def solve2(t, seq) do
    ...
    input = String.to_charlist(seq)
    sweep({program, 0, 0}, input ++ 'n\n', [])
  end

  def sweep({program, pointer, base}, input, outputs) do
    case IntCode.run(program, pointer, input, base) do
      {output, {program, pointer, input, base}} ->
        sweep({program, pointer, base}, input, [output | outputs])
      {_program, _pointer, _base} ->
        List.first(outputs)
    end
  end
  ...
  def walk(map, _t) do
    {start, _} = Enum.find(map, fn {_coord, v} -> v == "^" end)
    path = step(map, start, :up, [])
    Regex.scan(~r/(F+|R|L)/, path)
    |> Enum.map(fn [_, s] ->
      if String.starts_with?(s, "F") do
        s |> String.length |> to_string
      else
        s
      end
    end)
  ...
  def step(map, from, direction, path) do
    if (to = next(from, direction)) && map[to] == "#" do
      step(map, to, direction, ["F" | path])
    else
      case turn(map, from, direction) do
        {turn, direction} ->
          step(map, from, direction, [turn | path])
        nil ->
  ...
  def next(from, direction) do
    case {from, direction} do
      {{x, y}, :up} -> {x, y - 1}
      {{x, y}, :down} -> {x, y + 1}
      {{x, y}, :left} -> {x - 1, y}
      {{x, y}, :right} -> {x + 1, y}
  ...
  def turn(map, from, direction) do
    possible_turns =
      case direction do
        :up -> [{"L", :left}, {"R", :right}]
        :down -> [{"R", :left}, {"L", :right}]
    ...
    Enum.find_value(possible_turns, fn {turn, direction} ->
      map[next(from, direction)] == "#" && {turn, direction}
    end)
  end
end
...
seq =
  ...
  R,12,L,10,R,6,L,10
  """
IO.inspect Solver.solve1(t), label: "part1"
IO.inspect Solver.solve2(t, seq), label: "part2"
